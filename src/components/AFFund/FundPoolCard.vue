...
<template>
  <div class="fund-pool-card">
    <!-- 金额卡片 -->
    <div class="bg-[#F8FAFC] rounded-2xl px-10 py-8 mb-6 text-center">
      <div class="text-4xl font-bold text-[#0F172A] mb-3">${{ formattedAmount }}</div>
      <div class="flex items-center justify-center gap-2 text-sm">
        <span class="text-[#64748B]">{{ t('affund.expectedReturn') }}</span>
        ...
        <span class="text-[#12D18E] font-bold text-sm">{{ expectedReturn }}%</span>
      </div>
    </div>
    <!-- 进度条 -->
    <div class="mb-6">
      <div class="flex items-center gap-4">
        <span class="text-sm text-[#12D18E] font-semibold whitespace-nowrap">{{ t('affund.completedReturns') }}</span>
        <div class="flex items-center gap-4 flex-1">
          <div class="h-2 bg-[#E2E8F0] rounded-full overflow-hidden flex-1">
            <div class="h-full bg-[#12D18E] rounded-full transition-all duration-300" :style="{ width: progress + '%' }" />
          </div>
          <span class="text-sm text-[#12D18E] font-semibold">{{ progress }}%</span>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
      <div class="flex items-center gap-3 bg-white rounded-xl p-4">
        <div class="w-10 h-10 bg-[#E8FAF3] rounded-full flex items-center justify-center flex-shrink-0">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M16 2V6" stroke="#12D18E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M8 2V6" stroke="#12D18E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M3 9H21" stroke="#12D18E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path fill-rule="evenodd" clip-rule="evenodd"
              d="M19 4H5C3.895 4 3 4.895 3 6V19C3 20.105 3.895 21 5 21H19C20.105 21 21 20.105 21 19V6C21 4.895 20.105 4 19 4Z"
              stroke="#12D18E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </div>
        <div>
          <div class="text-xs text-[#64748B]">{{ t('affund.profitStartTime') }}</div>
          <div class="text-sm text-[#0F172A] font-semibold">{{ startDate }}</div>
        </div>
      </div>
      <div class="flex items-center gap-3 bg-white rounded-xl p-4">
        <div class="w-10 h-10 bg-[#E8FAF3] rounded-full flex items-center justify-center flex-shrink-0">
        ...
        <div>
          <div class="text-xs text-[#64748B]">{{ t('affund.profitEndTime') }}</div>
          <div class="text-sm text-[#0F172A] font-semibold">{{ endDate }}</div>
        </div>
      </div>
    </div>
