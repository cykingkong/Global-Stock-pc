      ...
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center">
          ...
          >
            ...
            />
            <path
              d="M4 12C4 10.34 5.34 9 7 9C8.66 9 10 10.34 10 12C10 13.66 8.66 15 7 15C5.34 15 4 13.66 4 12Z"
              fill="white"
            />
          </svg>
        </div>
  ...
  <div v-if="isMenuOpen" class="fixed inset-0 bg-white z-40 pt-16 lg:hidden overflow-y-auto">
    ...
    <nav class="px-6 pb-6">
      <a
        ...
        :key="link.name"
        :href="link.href"
        class="block py-3 text-lg font-medium text-gray-900 hover:text-blue-500 transition-colors"
        @click="toggleMenu"
      >
        {{ link.name }}
      </a>
    ...
    <div class="px-6 pt-6 pb-8">
      <button
        @click="handleLogout"
        class="w-full py-3 mb-4 text-base font-medium text-gray-900 bg-blue-500 text-white rounded-full transition-colors"
      >
        Sign up
      </button>
      <button
